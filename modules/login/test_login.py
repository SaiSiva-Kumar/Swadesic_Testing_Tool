...
def test_TC_LOGIN_002_send_otp_without_email(driver):
    """
    Test Case ID: TC_LOGIN_002
    Title: Send OTP Without Entering Email

    Objective:
    To verify that the app displays an error message when
    Send OTP is clicked without entering an email address.

    Test Steps:
    1. Click on See All Options button.
    2. Click on Continue with Email and OTP button.
    3. Click on Send OTP button without entering email.
    4. Verify error message is displayed.

    Expected Result:
    App displays "Please enter an email" error message.
    """
    ...
    executor.step(
        "Click on Send OTP button without entering email",
        login_page.click_send_otp
    )

    executor.step(
        "Verify error message is displayed",
        login_page.is_email_error_displayed
    )
